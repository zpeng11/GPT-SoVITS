...
from AR.models.t2s_model_onnx import Text2SemanticDecoder, T2SStageDecoder, T2SFirstStageDecoder, sample
...
class T2SInitStage(nn.Module):
    def __init__(self, t2s:Text2SemanticDecoder, vits:SynthesizerTrn):
        super().__init__()
        self.encoder = t2s.onnx_encoder
        self.first_stage_decoder = t2s.first_stage_decoder
        self.vits = vits
    ...
    def forward(self, ref_seq, text_seq, ref_bert, text_bert, ssl_content):
        ...
        k, v, y_emb, x_example, logits = self.first_stage_decoder(x, prompt)
        return [k, v, y_emb, x_example, logits, prompt]
...
class T2SModel(nn.Module):
    ...
    def forward(self, ref_seq, text_seq, ref_bert, text_bert, ssl_content):
        k, v, y_emb, x_example, logits, prompt = self.init_stage(ref_seq, text_seq, ref_bert, text_bert, ssl_content)
        y = sample(logits, prompt[0], top_k=15, top_p=1.0, repetition_penalty=1.35, temperature=1.0)[0].unsqueeze(0)
        for idx in range(1, 5):
            k = torch.nn.functional.pad(k, (0, 0, 0, 0, 0, 1))
            v = torch.nn.functional.pad(v, (0, 0, 0, 0, 0, 1))
            y_emb = torch.nn.functional.pad(y_emb, (0, 0, 0, 1, 0, 0))
            y = torch.nn.functional.pad(y, (0, 1, 0, 0))
            k_increasement, v_increasement, y_emb_increasement, logits = self.stage_decoder(y, k, v, y_emb, x_example)
            # y_sample = sample(logits, y[0], top_k=15, top_p=1.0, repetition_penalty=1.35, temperature=1.0)[0].unsqueeze(0)
            # y[0, -1] = y_sample[0, -1]
            k[-1:] = k_increasement
            v[-1:] = v_increasement
    ...
    def export(self, ref_seq, text_seq, ref_bert, text_bert, ssl_content, project_name):
        torch.onnx.export(
            ...
            output_names=['k', 'v', 'y_emb', 'x_example', 'logits', 'prompts'],
            dynamic_axes={
        ...
        simplify_onnx_model(f"onnx/{project_name}/{project_name}_t2s_fsdec.onnx")
        k, v, y_emb, x_example, logits, prompt = self.init_stage(ref_seq, text_seq, ref_bert, text_bert, ssl_content)
        y = sample(logits, prompt[0], top_k=15, top_p=1.0, repetition_penalty=1.35, temperature=1.0)[0].unsqueeze(0)
        ...
        torch.onnx.export(
            ...
            output_names=['k_increasement', 'v_increasement', 'y_emb_increasement', 'logits'],
            dynamic_axes={
                "iy": {1: "iy_length"},
                "ik": {0: "ik_length"},
                "iv": {0: "iv_length"},
...
class GptSoVitsV1V2():
    def __init__(self, vits:VitsV1V2Model, t2s:T2SModel):
        super().__init__()
        self.vits = vits
        self.t2s = t2s
...
def export(vits_path, gpt_path, project_name, model_version, export_audio_preprocessor=True, export_half_precision=True):
    vits = VitsV1V2Model(vits_path, version=model_version)
    ...
    ref_seq = torch.LongTensor(
        [
            cleaned_text_to_sequence(
                ...
                version='v1' if model_version.lower() in ['v1', 'v1.0'] else 'v2',
            )
        ]
    )
    ...
    ref_bert = torch.randn((ref_seq.shape[1], 1024))
    text_bert = torch.randn((text_seq.shape[1], 1024))
    ref_audio32k = torch.randn((1, 32000 * 5)) - 0.5 # 5 seconds of dummy audio

    os.makedirs(f"onnx/{project_name}", exist_ok=True)

    [ssl_content, spectrum, sv_emb] = preprocessor(ref_audio32k)

    gpt_sovits.export(ref_seq, text_seq, ref_bert, text_bert, ssl_content, spectrum, sv_emb, project_name)
    ...
    if export_half_precision:
        convert_onnx_to_half(f"onnx/{project_name}/{project_name}_t2s_fsdec.onnx")
        convert_onnx_to_half(f"onnx/{project_name}/{project_name}_t2s_sdec.onnx")
